      ...
      <div class="col-12 grid-margin stretch-card">
        <div class="card">
          <div class="card-body">
            ...
            <div v-if="pending_earlytimes.length > 0">
              <table class="table table-bordered table-responsive-md text-center">
                ...
                <tbody>
                  <IndexDetail v-for="(earlytime, index) in pending_earlytimes" :key="index" :earlytime="earlytime" @change-disabled="changeDisabled"></IndexDetail>
                </tbody>
              </table>
              <div class="text-right mt-3">
                <form id="form" action="/earlytime/many/approve" method="post" ref="form">
...
export default {
  props: ['month', 'pending_earlytimes'],
  mixins: [Data, Link],
  components:{
    IndexDetail
  },
  ...
  methods: {
    monthChange(){
      ...
      window.location = `/earlytime?year=${y}&month=${m}`
    },
    changeDisabled(){
      this.disabled = true
      document.getElementsByName('earlytime_ids[]').forEach(elm => {
        if(elm.checked){
          this.total++
          this.disabled = false
        }
