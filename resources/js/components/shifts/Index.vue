          ...
          <div class="card-body">
            ...
            <div v-if="pending_shifts.length > 0">
              <table class="table table-bordered table-responsive-md text-center">
                ...
                <tbody>
                  <IndexDetail v-for="(shift, index) in pending_shifts" :key="index" :shift="shift" @change-disabled="changeDisabled"></IndexDetail>
                </tbody>
              </table>
              <div class="text-right mt-3">
                <form id="form" action="/shift/many/approve" method="post" ref="form">
...
export default {
  ...
  methods: {
    monthChange(){
      ...
      window.location = `/shift?year=${y}&month=${m}`
    },
    changeDisabled(){
      this.disabled = true
      document.getElementsByName('shift_ids[]').forEach(elm => {
        if(elm.checked){
          this.total++
          this.disabled = false
        }
